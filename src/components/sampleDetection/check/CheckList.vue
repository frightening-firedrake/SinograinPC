...
//		console.log(this.$route.query)
		...
		this.$root.eventHub.$off("inputlistitem")
		...
		//	监听列表点击录入事件
		this.$root.eventHub.$on("inputlistitem", function(id) {
//					console.log("inputlistitem",id)
			this.$router.push({ path: '/index/sampleDetection/checkList/checkInput', query: { libid: id } })
...
//			console.log(id)
			this.$router.push({ path: '/index/sampleDetection/checkList/checkEdit', query: { libid: id } })
		}.bind(this));
	},
	destroy() {
		this.$root.eventHub.$off("inputlistitem")
...
//			this.connect();
		...
		scanCode() {
			...
			this.$router.push({ path: '/index/sampleDetection/checkList/checkInput' })
		},
		connect() {
...
//				打印样品领取交接单触发
	...
	data() {
		return {
			datalistURL: '/liquid/role13/data',
